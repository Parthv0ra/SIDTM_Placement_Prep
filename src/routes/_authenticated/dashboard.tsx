  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [sessions, scorecards] = await Promise.all([
        supabase.from("interview_sessions").select("*").order("created_at", { ascending: false }).limit(20),
  ...
  return (
    <AppShell title="Dashboard" subtitle="Your placement readiness at a glance">
      <div className="grid gap-4 md:grid-cols-3">
        <StatCard icon={<Target className="h-4 w-4" />} label="Latest readiness" value={latest ? `${latest.overall_score}/100` : "—"} />
        <StatCard icon={<Trophy className="h-4 w-4" />} label="Best score" value={best ? `${best}/100` : "—"} />
        <StatCard icon={<Clock className="h-4 w-4" />} label="Sessions completed" value={`${sessions.filter((s) => s.status === "completed").length}`} />
      </div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-3">
        <Card className="md:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0">
            ...
            <Button asChild size="sm"><Link to="/new"><PlusCircle className="mr-1.5 h-4 w-4" />New interview</Link></Button>
          </CardHeader>
          <CardContent className="h-64">
            {chart.length === 0 ? (
              <div className="flex h-full items-center justify-center text-sm text-muted-foreground">Complete an interview to see your trend</div>
            ) : (
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Category snapshot</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {latest ? Object.entries(latest.category_scores as Record<string, number>).map(([k, v]) => (
              <div key={k}>
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          {sessions.length === 0 ? (
            <p className="text-sm text-muted-foreground">You haven't started any interviews yet. Click "New interview" to begin.</p>
          ) : (
            <ul className="divide-y">
              {sessions.slice(0, 5).map((s) => (
                <li key={s.id} className="flex items-center justify-between py-3">
                  <div>
                    <p className="text-sm font-medium">{s.company} — {s.role}</p>
