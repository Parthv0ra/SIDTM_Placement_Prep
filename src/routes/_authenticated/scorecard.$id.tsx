    ...
    >
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            <ul className="list-disc space-y-1.5 pl-5 text-sm">
              {((data.card.strengths as string[]) ?? []).map((s, i) => (
                <li key={i}>{s}</li>
              ))}
            </ul>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <ul className="list-disc space-y-1.5 pl-5 text-sm">
              {((data.card.recommendations as string[]) ?? []).map((s, i) => (
                <li key={i}>{s}</li>
              ))}
            </ul>
          </CardContent>
          ...
          <CardContent className="pt-4">
            {data.referenceSolution ? (
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <h4 className="text-xs font-semibold text-foreground">
                    Expected Value Range & Sanity Check
                  </h4>
                  <p className="text-xs text-muted-foreground leading-relaxed whitespace-pre-wrap rounded-md bg-secondary/30 p-3 border">
          ...
          {data.questions.map((q, i) => {
            ...
            return (
              <div key={q.id} className="rounded-md border p-4">
                ...
                {r?.transcript && (
                  <p className="mt-2 rounded bg-secondary/40 p-2 text-xs text-muted-foreground">
                    {r.transcript}
                  </p>
                )}
                {scores && (
                  <div className="mt-2 flex flex-wrap gap-2 text-xs">
                    {Object.entries(scores)
                      ...
                      .map(([k, v]) => (
                        <span key={k} className="rounded-full bg-secondary px-2 py-0.5 capitalize">
                          {k.replace("_", " ")}: <b>{v}</b>
                        </span>
                      ))}
                  </div>
                )}
                {scores && typeof (scores as any).feedback === "string" && (
                  <p className="mt-2 text-sm font-normal text-muted-foreground leading-relaxed">
                    {(scores as any).feedback}
                  </p>
                )}
                ...
                {q.category === "behavioral" && (scores as any)?.star_structure && (
                  <div className="mt-3 p-3 rounded-md bg-secondary/10 border border-border/60 space-y-2">
                    <div className="text-xs font-semibold text-foreground flex items-center gap-1">
                      <CheckCircle2 className="h-3.5 w-3.5 text-blue-500" /> Behavioral Answer
                      ...
                      {Object.entries((scores as any).star_structure).map(([component, exists]) => (
                        <div key={component} className="flex items-center gap-1.5">
                          {exists ? (
                            ...
                            <span className="w-3.5 h-3.5 rounded-full border border-destructive/40 bg-destructive/10 flex items-center justify-center text-[10px] font-bold text-destructive shrink-0">
                              ✕
                            </span>
                          )}
                          <span
      ...
      <div className="mt-6 flex gap-2">
        <Button asChild>
          <Link to="/new">Practice again</Link>
        </Button>
