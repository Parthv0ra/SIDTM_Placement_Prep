...
function Admin() {
  ...
  const studentsList = Object.values(studentMap).sort((a, b) => b.sessions.length - a.sessions.length);

  // Render Student Detail View
  if (selectedStudentId && studentMap[selectedStudentId]) {
    const student = studentMap[selectedStudentId];
    return (
      <AppShell title={`Student Profile · ${student.name}`} subtitle="Individual progress and mock interview history">
        <div className="mb-6">
          <Button variant="ghost" onClick={() => setSelectedStudentId(null)} className="gap-2 text-sm pl-0 hover:bg-transparent">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <StatCard label="Average readiness" value={student.scorecards.length ? `${student.avgReadiness}/100` : "—"} />
        </div>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-3">
          <Card className="md:col-span-1">
            ...
            <CardContent className="text-sm space-y-2">
              <div><span className="text-muted-foreground font-medium block">Full Name</span> {student.name}</div>
              <div><span className="text-muted-foreground font-medium block mt-1">Email Address</span> {student.email}</div>
            </CardContent>
          </Card>
          ...
          <Card className="md:col-span-2">
            <CardHeader>
              ...
              <CardDescription>Mock interviews and case studies taken by this student</CardDescription>
            </CardHeader>
            <CardContent>
              <Table>
      ...
      {activeTab === "students" ? (
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>Select a student to view their personal dashboard and individual sessions</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader><TableRow>
              ...
              <TableBody>
                {studentsList.map((student) => (
                  <TableRow key={student.id}>
                    ...
                    <TableCell className="text-right">
                      <Button size="xs" variant="outline" className="h-7 px-2.5 text-[11px] font-medium" onClick={() => setSelectedStudentId(student.id)}>
                        View Details
                      </Button>
                    </TableCell>
                  </TableRow>
